...
def collide(x1, y1, x2, y2, x3, y3, x4, y4):  # BENTUK YG DITABRAK (TEMBOK)
...
def collide2(x1, y1, x2, y2, x3, y3, x4, y4, size):  # BENTUK YG DITABRAK (APEL)
...
class snake():  # MEMBUAT CLASS SNAKE/ULAR
    def __init__(self, speed, size):
        self.pos = [20, 20]  # POSISI AWAL SI ULAR
        self.image = pg.Surface((10*size, 10*size))  # KEPALA PUNYA ULAR
        self.image.fill((255, 255, 255))  # WARNA KEPALA ULAR
        self.speed = speed
        self.size = size
        self.images = []
        self.old_pos = [[20, 20]]
    ...
    def right(self):  # ARAH KLIK KEKANAN
        self.direction = [self.speed, 0]

    def left(self):  # ARAH KLIK KEKIRI
        self.direction = [-self.speed, 0]

    def up(self):  # ARAH KLIK KEATAS
        self.direction = [0, -self.speed]

    def down(self):  # ARAH KLIK KEBAWAH
        self.direction = [0, self.speed]
    ...
    def check_collisions(self, x):  # PROSES TABRAKAN SAMA TEMBOK
        c = collide(self.pos[0], self.pos[1], self.pos[0]+10,
                    self.pos[1]+10, x[0], x[1], x[0]+10, x[1]+10)
        return c

    def check_apple(self, x):  # PROSES TABRAKAN SAMA APEL
        c = collide2(self.pos[0], self.pos[1], self.pos[0]+10,
                     self.pos[1]+10, x[0], x[1], x[0]+10, x[1]+10, self.size)
        return c

    def check_collisions2(self, x):  # PROSES TABRAKAN SAMA BADAN ULAR
        c = collide3(self.pos[0], self.pos[1], self.pos[0]+10,
                     self.pos[1]+10, x[0], x[1], x[0]+10, x[1]+10, self.size)
        return c

    def add_apple(self):  # KETIKA MEMAKAN APEL
        self.score += 1  # PENAMBAHAN SKOR KALO ABIS MAKAN APEL
        # UKURAN BADAN YANG DITAMBAHKAN
        block = pg.Surface((10*self.size, 10*self.size))
        block.fill((0, 255, 0))  # WARNA PENAMBAHAN BADAN
        self.images.append([block, [10, 10]])  # PENAMBAHAN PANJANG ULAR


class apple():  # CLASS APEL/MAKANAN
    def __init__(self, size):
        self.pos = [random.randrange(10, 780, 10),  # MAKANAN DIACAK
                    random.randrange(10, 430, 10)]  # APEL DI DALAM PAPAN PERMAINAN
        self.image = pg.Surface((10*size, 10*size))  # UKURAN MAKANAN
        self.image.fill((255, 0, 0))  # WARNA MAKANAN APEL

        # FUNGSI DEF BUAT MAKANAN BONUS


class game():  # CLASS PERMAINAN
    def __init__(self, speed, size=1):
        ...
        self.apple = apple(size)  # POSISI APEL DIDALAM DINDING
    ...
    def over(self):  # KETIKA PERMAINAN BERAKHIR
        while 1:
            ...
            txts = pg.font.SysFont('Courier New', 50).render(
                'Game Over    Score:', True, (255, 255, 255))  # TAMPILAN KETIKA PERMAINAN SELESAI DENGAN FONT COURIER
            txtrect = txts.get_rect()
            # JARAK TULISAN POSISI DARI KIRI DAN ATAS
            txtrect.topleft = (20, 150)
            self.screen.blit(txts, txtrect)
            txts = pg.font.SysFont('Courier New', 50).render(
                str(self.snake.score), True, (255, 255, 255))  # UNTUK TAMPILAN NILAI SKOR
            txtrect = txts.get_rect()
            # JARAK TULISAN POSISI DARI KIRI DAN ATAS
            txtrect.topleft = (600, 150)
            self.screen.blit(txts, txtrect)
            pg.display.update()
            self.make_button((153, 300, 100, 50), 'Restart', [  # JARAK DAN UKURAN (KIRI, ATAS, LEBAR, TINGGI)
                             (255, 255, 255), (150, 150, 150)], action=lambda: restart())
            if self.hover == True:
    ...
    def loop(self):
        ...
        while self.game_over != True:
            # WARNA BACKGOUND SAAT GAME DIMULAI
...
class startmenu():
    def __init__(self):
        ...
        self.screen = pg.display.set_mode((800, 450))
        self.b1 = '(150, 300,100,50),"New Game", [(0,255,0), (0,150,0)], action = self.start'
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        rectangle.fill((255, 255, 255, alpha), special_flags=pg.BLEND_RGBA_MIN)
        self.screen.blit(rectangle, pos)
        self.make_text((pos[0]+pos[2]/2), (pos[1]+pos[3]/2),
                       text, a=True, size=textsize)

    def mainloop(self):
        while 1:
            self.screen.fill((35, 38, 117))  # WARNA BACKGROUND MENU AWAL
            self.make_text(400, 150, 'Game Snake Optimus',  # JARAK TULISAN
                           color=(255, 255, 255), size=50, a=True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
    ...
    def start(self):  # Menu pilihan untuk tampilan ukuran game
    ...
    def start3(self):  # Menu pilihan untuk level game pada pilihan Normal
    ...
    def start4(self):  # Menu pilihan untuk level game pada pilihan Big
        self.size = 2
        self.b1 = '(150, 300,100,50),"Easy", [(0,255,0), (0,150,0)], action = self.e'
        self.b2 = '(283, 300,100,50),"Normal", [(0,255,0), (0,150,0)], self.n'
        self.b3 = '(417, 300,100,50),"Hard", [(0,255,0), (0,150,0)], action = self.h'
    ...
    def e(self):  # KECEPATAN MODE EASY
        start(0.25, self.size)

    def n(self):  # KECEPATAN MODE NORMAL
        start(0.5, self.size)

    def h(self):  # KECEPATAN MODE HARD
        start(1, self.size)

    def ex(self):  # KECEPATAN MODE EXPERT
        start(2, self.size)
...
def start(speed, size):  # FUNGSI START PADA GAME YANG MENGGUNAKAN FUNGSI GLOBAL
    global g, m
    del m
...
def restart():  # FUNGSI RESTART YANG MENGGUNAKAN FUNGSI GLOBAL
    global g
    del g
    menu()
